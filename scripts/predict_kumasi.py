...
def predict_kumasi():
    print("Running Yield Prediction for Kumasi (2024)...")
    ...
    # Find Kumasi
    loc = session.query(Location).filter(Location.name.like('%Kumasi%')).first()
    if not loc:
        print("Error: Kumasi location not found in DB.")
        return

    print(f"Target Location: {loc.name} (ID: {loc.id}, Lat: {loc.latitude}, Lon: {loc.longitude})")
    
    # Init Layers
    # Using LightGBM and Standard Environmental Layer
    ...
    if not data:
        print("No data found for Kumasi in 2025.")
        return
    ...
    print("\n" + "="*40)
    print("PREDICTION RESULTS: KUMASI")
    print("="*40)
